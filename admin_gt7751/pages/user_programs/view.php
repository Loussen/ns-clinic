...
$page_title="Proqram istifadəçiləri";
?>
					...
					<table class="table table-striped">
						<thead>
							<tr class="auto_resize">
								...
								<th style="width:50%">Son tarix</th>
						...
						{
							if($row["expire_time"]<=$time) $stl='style="color:red;font-weight:bold;"'; else $stl='';
							$addButtons=array('<a href="index.php?do=user_programs&login_this='.$row["id"].'" data-toggle="tooltip" data-animation="false" data-original-title="Bu adla daxil ol" class="m-r-10"> <i class="fa fa-sign-in fa-lg"></i></a>');
							
							echo '<tr '.$stl.'>
									<td>'.checkbox_row($row["id"]).' '.$row["id"].'</td>
									<td>'.decode_text($row["login"]).'</td>
									<td>'.decode_text($row["name"]).'</td>
									<td>'.decode_text($row["email"]).'</td>
									<td>'.date("d.m.Y H:i",$row["expire_time"]).'</td>
