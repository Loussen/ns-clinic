...
{
	$datas_post=array('name','short_text','full_text','link');
	...
	if($error==''){
		...
		if($edit>0) mysqli_query($db,"update $do set $query_update,link='$link' where id='$edit' ");
		else mysqli_query($db,"insert into $do (link,position,active,$query_insert) values ('$link','$position','$active',$query_insert_val) ");

//		echo "insert into $do (link,position,active,$query_insert) values ('$link','$position','$active',$query_insert_val)"; exit;
