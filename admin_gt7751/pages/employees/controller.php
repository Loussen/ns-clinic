...
if(isset($_GET["category_show"])) $category_show=safe($_GET["category_show"]); else $category_show=0;
if(mysqli_num_rows(mysqli_query($db,"select id from categories where id='$category_show' "))==0) $category_show=0;
...
if(isset($_POST["submit_insert_update"]) && check_csrf_(safe($_POST["csrf_"]),$do) )
{
	...
	if($error==''){
		...
		if($edit>0) mysqli_query($db,"update $do set $query_update,category_id='$category_id' where id='$edit' ");
		else mysqli_query($db,"insert into $do (category_id,active,$query_insert) values ('$category_id','$active',$query_insert_val) ");
		...
		if($uploadedFile!=''){
			makeThumb($imageFolder.'/'.$uploadedFile,$imageFolder.'/thumb_'.$uploadedFile,120,120);
		}
	}
}
