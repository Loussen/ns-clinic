...
{
	$datas_post=array('name','text','period','percent');
	...
	if($error==''){
		...
		if($edit>0) mysqli_query($db,"update $do set $query_update,period='$period',percent='$percent' where id='$edit' ");
		else mysqli_query($db,"insert into $do (position,active,period,percent,$query_insert) values ('$position','$active','$period','$percent',$query_insert_val) ");
