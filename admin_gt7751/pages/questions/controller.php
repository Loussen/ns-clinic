...
{
	$datas_post=array('datetime','hour','tip','answer','question');
	...
	if($error==''){
		...
		if($parent_id>0){
			if($edit>0) mysqli_query($db,"update $do set $query_update,datetime='$datetime',parent_id='$parent_id' where id='$edit' ");
			else mysqli_query($db,"insert into $do (parent_id,datetime,position,active,$query_insert) values ('$parent_id','$datetime','$position','$active',$query_insert_val) ");
		}else{
			if($edit>0) mysqli_query($db,"update $do set $query_update,tip='$tip',datetime='$datetime' where id='$edit' ");
			else mysqli_query($db,"insert into $do (datetime,tip,position,active,$query_insert) values ('$datetime','$tip','$position','$active',$query_insert_val) ");
		}
