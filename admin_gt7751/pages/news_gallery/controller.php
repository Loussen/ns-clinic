		...
		foreach($images as $image){
			...
			if($image!=''){
				if(!checkFileType($imageName)) $error='Şəkil tipi uyğun deyil.';
					...
					makeThumb($uploadImage,$imageFolder.'/thumb_'.$imageName,400,250);
				}
				$count++;
			}
			if($edit>0){
				mysqli_query($db,"update $do set $query_update,parent_id='$parent_id',video_url='$video_url',image='$imageName' where id='$edit' ");
				break;
			}
			else mysqli_query($db,"insert into $do (parent_id,image,video_url,position,active,$query_insert) values ('$parent_id','$imageName','$video_url','$position','$active',$query_insert_val) ");
		}
