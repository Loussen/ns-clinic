					...
					<table class="table table-striped">
						<thead>
							<tr>
								...
								<th style="width:50%">Şərhi</th>
								<th style="width:15%">Tarix</th>
								<th style="width:15%" class="print_hide">Alətlər</th>
							</tr>
						</thead>
						<tbody>
						<?php
						...
						{
							echo '<tr>
									<td>'.checkbox_row($row["id"]).' '.decode_text($row["name"]).'-'.decode_text($row["table_name"]).'</td>
									<td>'.decode_text($row["comment"]).'</td>
									<td>'.date("d.m.Y H:i",$row["datetime"]).'</td>
