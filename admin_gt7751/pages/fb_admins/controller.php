...
{
	$datas_post=array('fb_id');
	include "pages/__tools/check_post_datas.php";

	if($error==''){
		include "pages/__tools/create_only_langs_query.php";
		
		if($edit>0) mysqli_query($db,"update $do set fb_id='$fb_id' where id='$edit' ");
		else mysqli_query($db,"insert into $do (fb_id,active) values ('$fb_id','$active') ");
