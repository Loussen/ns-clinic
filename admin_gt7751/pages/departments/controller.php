...
{
	$datas_post=array('name','short_text','full_text');
...
//	if($$Name=='') $error='Ad daxil edilməyib. (Dil: '.$lang_name.')';
	...
	if($error==''){
		...
		if($edit>0) mysqli_query($db,"update $do set $query_update where id='$edit' ");
		    ...
		    mysqli_query($db,"insert into $do (position,active,$query_insert) values ('$position','$active',$query_insert_val) ");
        }
		...
		$uploadedFileIcon=fileUpload('image_icon');
		if($uploadedFile!=''){
			makeThumb($imageFolder.'/'.$uploadedFile,$imageFolder.'/thumb_'.$uploadedFile,370,317);
		}
	}
}
elseif($delete>0 && mysqli_num_rows(mysqli_query($db,"select id from $do where id='$delete' "))>0)
{
	...
	if(in_array('important',$get_table_name_columns) && $info["important"]==1) $error="Bu məlumatı silmək olmaz.";
		...
		if(isset($imageFolder)) deleteOldFiles($do,$delete,$imageFolder,'image_icon');
